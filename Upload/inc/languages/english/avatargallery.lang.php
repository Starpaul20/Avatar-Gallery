<?php
/**
 * Avatar Gallery
 * Copyright 2014 Starpaul20
 */

$l['avatar_gallery'] = "Avatar Gallery";
$l['avatargallery_info_name'] = "Avatar Gallery";
$l['avatargallery_info_desc'] = "Adds an avatar gallery to your board.";

$l['error_noavatar'] = "You did not choose an avatar. Please go back and do so now.";
$l['default_gallery'] = "Default Gallery";
$l['avatars_in_gallery'] = "Avatars in {1}";

$l['local_galleries'] = "Local Avatar Galleries";
$l['gallery'] = "Gallery:";
$l['change_avatar'] = "Change Avatar";
$l['local_galleries'] = "Local Avatar Galleries";
